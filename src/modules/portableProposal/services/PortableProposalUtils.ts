...
const log = getLogger("PortableProposalUtils")

export class PortableProposalUtils {
    static generateProposal(unsignedProposal) {
        if (unsignedProposal?.attributes?.customPayload?.proposal) {
            ...
            proposal.policy_data = PortableProposalUtils.generatePolicyData(
                contractNumber,
                PortableProposalUtils.generateDateProposal(unsignedProposal.date)
            )
            proposal.policyholder_data = PortableProposalUtils.generatePolicyHolderData()
            proposal.variable_policy_data = PortableProposalUtils.generateVariablePolicyData(
                contractNumber,
                unsignedProposal.amount
            )
            proposal.charge_type_data = PortableProposalUtils.generateChargeData(unsignedProposal.amount)
            return PortableProposalUtils.formatedProposalValues(proposal)
        }
        return null
    }
    ...
    static formatedProposalValues(proposal) {
        ...
        proposal.insured_data.date_of_birth = moment(proposal.insured_data.date_of_birth, "DD/MM/YYYY").format("MMDDYYYY")
